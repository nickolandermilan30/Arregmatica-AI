...
"${text}"
      ...
      const correctPercent = total > 0 ? Math.round((parsed.correct / total) * 100) : 0;
      const wrongPercent = total > 0 ? Math.round((parsed.wrong / total) * 100) : 0;
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-indigo-100 flex flex-col">
      ...
      <header className="bg-indigo-600 text-white py-4 shadow-md">
        <div className="max-w-6xl mx-auto px-6 flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold tracking-wide">Essay Checker</h1>
        </div>
      </header>
      ...
      <main className="flex-grow flex items-center justify-center px-6 py-10">
        <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            <h2 className="text-lg font-semibold text-gray-700 mb-3">✍️ Paste your essay</h2>
            <textarea
            ...
            >
              {loading ? <><Loader2 className="w-5 h-5 animate-spin" /> Analyzing...</> : "🔍 Check Essay"}
            </button>
          </div>
          ...
          <div className="flex flex-col bg-white rounded-2xl shadow-lg p-6 border border-gray-100 min-h-[350px]">
            ...
            {!analysis && !loading && (
              <p className="text-gray-400 italic flex-grow">📊 Your essay analysis will appear here...</p>
            )}

            {analysis && (
              <div className="flex flex-col gap-4 flex-grow">
                <p className="text-gray-700">✅ Correct Sentences: <span className="font-bold">{analysis.correct}</span></p>
                <p className="text-gray-700">❌ Wrong Sentences: <span className="font-bold">{analysis.wrong}</span></p>

                {/* Correct Progress */}
                <div>
                  <p className="text-sm text-gray-600 mb-1">Correct ({analysis.correctPercent}%)</p>
                  <div className="w-full bg-gray-200 rounded-full h-3">
                    <div className="bg-green-500 h-3 rounded-full transition-all duration-500" style={{ width: `${analysis.correctPercent}%` }} />
                  </div>
                </div>

                {/* Wrong Progress */}
                <div>
                  <p className="text-sm text-gray-600 mb-1">Wrong ({analysis.wrongPercent}%)</p>
                  <div className="w-full bg-gray-200 rounded-full h-3">
                    <div className="bg-red-500 h-3 rounded-full transition-all duration-500" style={{ width: `${analysis.wrongPercent}%` }} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-4">
                    <div className="bg-indigo-500 h-4 rounded-full transition-all duration-500" style={{ width: `${analysis.overall}%` }} />
                  </div>
                  <p className="text-sm text-gray-700 mt-1 font-medium">{analysis.overall}% Score</p>
                </div>

                {/* Feedback */}
                <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-3 mt-2">
                  <p className="text-gray-700 text-sm italic">{analysis.feedback}</p>
                </div>
